...
export default function DownloadEntry() {
  ...
  useEffect(() => { if (editId) fetch(`${API_BASE_URL}/download_list.php`, { credentials: "include" }).then((r) => r.json()).then((d) => { const item = (d.data || []).find((entry) => String(entry.id) === String(editId)); if (item) setForm({ title: item.title || "", description: item.description || "" }); }); }, [editId]);
